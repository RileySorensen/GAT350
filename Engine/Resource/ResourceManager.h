...
namespace neu
{
	...
	public:
		...
		template<typename T>
		void Add(const std::string& name, std::shared_ptr<Resource> resources);
	...
	template<typename T>
	inline void ResourceManager::Add(const std::string& name, std::shared_ptr<Resource> resource)
	{
		std::string lowerName = ToLower(name);
		m_resources[lowerName] = resource;
	}
